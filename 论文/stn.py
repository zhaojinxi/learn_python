...
def cnn(x):
    with tensorflow.variable_scope('cnn'):
        ...
        stw=tensorflow.get_variable('stw', [3,3,32,6], initializer=tensorflow.zeros_initializer)
        stb=tensorflow.get_variable('stb', initializer=numpy.array([[1., 0, 0], [0, 1., 0]]).astype('float32').flatten())
        stz=tensorflow.nn.conv2d(z3,stw,[1,1,1,1],'VALID')+stb
        z3=transformer.spatial_transformer_network(z3, tensorflow.reshape(stz,[-1,6]),[3,3])
