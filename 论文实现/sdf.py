...
def DeformableConvolution(x, offset):
    grid=tensorflow.meshgrid(tensorflow.range(offset.shape[1]),tensorflow.range(offset.shape[2]))
    grid=tensorflow.stack([grid[1],grid[0]],-1)
    grid=tensorflow.expand_dims(grid,0)
    grid=tensorflow.tile(grid,[x.shape[0],1,1,x.shape[3]])
    ##############################删除#####################
    offset=tensorflow.stack([tensorflow.cast(offset[:,:,:,0],tensorflow.float32)+0.1,tensorflow.cast(offset[:,:,:,1],tensorflow.float32)+0.5],-1)
    offset=tensorflow.tile(offset,[1,1,1,x.shape[3]])
    ##############################删除#####################
...
def cnn(x):
    with tensorflow.variable_scope('cnn'):
        ...
        dcw=tensorflow.get_variable('dcw', [3,3,8,16], initializer=tensorflow.zeros_initializer)
        dcb=tensorflow.get_variable('dcb', 16, initializer=tensorflow.zeros_initializer)
        dcz=tensorflow.nn.conv2d(z1,dcw,[1,1,1,1],'SAME')+dcb
        l_offset = DeformableConvolution(z1, dcz)
