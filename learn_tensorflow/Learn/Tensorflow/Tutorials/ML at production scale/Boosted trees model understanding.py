...
# Load dataset.
dftrain = pd.read_csv('C:/Users/zhao/Downloads/titanic_train.csv')
dfeval = pd.read_csv('C:/Users/zhao/Downloads/titanic_eval.csv')
...
# Train the model.
est = tf.contrib.estimator.boosted_trees_classifier_train_in_memory(train_input_fn,feature_columns,**in_memory_params)
...
def dist_violin_plot(df_dfc, ID):
    ...
    parts=ax.violinplot([df_dfc[w] for w in ix],vert=False,showextrema=False,widths=0.7,positions=np.arange(len(ix)))
    ...
    ax.scatter(example,np.arange(example.shape[0]),color=sns.color_palette()[2],s=100,marker="s",label='contributions for example')
...
ax = dfc_mean[sorted_ix].plot(kind='barh', color=sns_colors[1], title='Mean |directional feature contributions|', figsize=(10, 6))
ax.grid(False, axis='y')
...
def create_bt_est(n_trees):
    return tf.estimator.BoostedTreesRegressor(fc, n_batches_per_layer=1, n_trees=n_trees)
