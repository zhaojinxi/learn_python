...
def gru(h_old,x):
    ...
    t=tensorflow.nn.tanh(tensorflow.matmul(tensorflow.concat(r*h_old,x),tw)+tb)
...
for i in range(5):
    if i==0:
        output_cell,output_hidden=Session.run(output,feed_dict={hidden:numpy.zeros((batch_size,hidden_dim)),input_data:numpy.ones((batch_size,input_dim))})
        last_cell=output_cell
        all_output.append(output_hidden)
    else:
        output_cell,output_hidden=Session.run(output,feed_dict={input_data:numpy.random.random_sample((batch_size,input_dim))})
        last_cell=output_cell
        all_output.append(output_hidden)